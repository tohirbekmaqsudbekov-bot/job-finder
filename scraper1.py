...
# Map raw OLX titles into canonical Kasb values
OCCUPATION_KEYWORDS = [
    ("Oshpaz", [r"\boshpaz\b", r"\bpovar\b", r"\bpazanda\b", r"\bsamosa\b", r"\bshashlik\b", r"\bbarbekyu\b", r"\bchef\b", r"\bosxona\b", r"\boshxonachi\b"]),
    ("O'qituvchi", [r"\bo['’`]?qituvchi\b", r"\bustoz\b", r"\bteacher\b", r"\bmentor\b", r"\btarbiyachi\b", r"\bbilim\b", r"\bkurs\b"]),
    ("Sotuvchi", [r"\bsotuvchi\b", r"\bprodavets\b", r"\bsavdo agent\b", r"\bsavdo vakili\b", r"\bkassir\b", r"\bmagazin\b", r"\bdo'kon\b", r"\bdo'konchi\b", r"\bsotuv\b"]),
    ("Gruzchik", [r"\bgruzchik\b", r"\byuk\b", r"\byuk ko['’`]?taruvchi\b", r"\bko'chirish\b", r"\bko'chirmachi\b"]),
    ("Kuryer", [r"\bkuryer\b", r"\bdostavka\b", r"\bdelivery\b", r"\bkur'er\b", r"\bdeliveryman\b"]),
    ("Santexnik", [r"\bsantexnik\b", r"\bsantexnika\b", r"\bplumber\b"]),
    ("Elektrik", [r"\belektrik\b", r"\belectrician\b"]),
    ("Mekhanik", [r"\bmexanik\b", r"\bmechanic\b"]),
    ("Usta", [r"\busta\b", r"\busta[si]*\b", r"\bmaster\b", r"\bmutaxassis\b"]),
    ("Ofitsiant", [r"\bofitsiant\b", r"\bofitsiantka\b", r"\bwaiter\b", r"\bwaitress\b"]),
    ("Administrator", [r"\badmin\b", r"\badministrator\b", r"\boperator\b", r"\bregistratur\b", r"\bregistrator\b", r"\bkonsultant\b"]),
    ("Buxgalter", [r"\bbuxgalter\b", r"\bbuhgalter\b", r"\baccountant\b", r"\bhisobchi\b"]),
    ("Haydovchi", [r"\bhaydovchi\b", r"\bdriver\b", r"\bqarovsiz\b"]),
    ("Ishchi", [r"\bishchi\b", r"\bworker\b"]),
    ("Nanny", [r"\bnyanya\b", r"\bvospitatel\b", r"\bbolalar\b", r"\bsad\b"]),
    ("IT", [r"\bdasturchi\b", r"\bprogrammist\b", r"\bdeveloper\b", r"\bfrontend\b", r"\bbackend\b", r"\bfullstack\b", r"\bpython\b", r"\bjava\b", r"\breact\b", r"\bnode\b", r"\bgolang\b", r"\bandroid\b", r"\biphone\b", r"\bmobile\b"]),
    ("Marketing", [r"\bsmm\b", r"\bmarketing\b", r"\btargetolog\b", r"\breklama\b", r"\bseo\b", r"\bbanner\b", r"\bbrand\b"]),
    ("HR", [r"\bhr\b", r"\bkadr\b", r"\bpersonal\b", r"\bmenedjer po personalu\b", r"\bhr-menedjer\b", r"\bhr menejer\b"]),
    ("Doktor", [r"\bshifokor\b", r"\bstomatolog\b", r"\bprotez\b", r"\btibbiyot\b", r"\btibbiy\b"]),
]

GENERIC_PHRASE_RE = re.compile(
    r"\b(ishga taklif qilinadi|ishga olamiz|ishga olinadi|ishga qabul qilinadi|kerak|ish bor|zudlik bilan|shoshilinch|ishga qabul qilamiz|taklif qilamiz)\b",
    flags=re.IGNORECASE,
)
...
def normalize_job_title(title: str) -> str:
    ...
    for canonical, patterns in OCCUPATION_KEYWORDS:
        for pat in patterns:
            if re.search(pat, title_text):
                return canonical

    title_text = GENERIC_PHRASE_RE.sub(" ", title_text)
    title_text = re.sub(r"[^\w\s\u0400-\u04FF\u0600-\u06FFʻʼ]+", " ", title_text)
    title_text = re.sub(r"\s+", " ", title_text).strip()

    if len(title_text) < 3 or title_text in ["ish", "ishchi", "kerak"]:
...
class Job:
    def __init__(self, raw_kasb: str, kasb: str, ish_haqi: str, joylashuv: str, ish_turi: str, bandlik_turi: str, telefon: str, link: str):
        self.raw_kasb = raw_kasb
        self.kasb = kasb
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "Raw Kasb": clean_text(self.raw_kasb),
    ...
    for page in range(1, max_pages + 1):
        ...
        for listing in listings:
            ...
            try:
                ...
                job = Job(
                    raw_kasb=raw_title,
                    kasb=normalize_job_title(raw_title),
                    ish_haqi=salary,
                    joylashuv=joylashuv,
                    ish_turi=ish_turi,
...
def main():
    ...
    fieldnames = ["Kasb", "Raw Kasb", "Ish haqi", "Joylashuv", "Ish turi", "Bandlik turi", "Telefon", "Link"]
